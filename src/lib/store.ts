...
export interface Progress {
  ...
  points: number;
  /** Purchased/earned unlock ids (sections, cosmetics). */
  unlocks: string[];
  /** Reward-milestone ids already claimed. */
  claimedRewards: string[];
  /** Active cosmetic accent-theme id. */
  accent: string;
}
...
function applyXp(progress: Progress, xp: number, coins: number): Progress {
  ...
  return {
    ...
    // Points are a lifetime score: they only ever go up, tracking every XP earned.
    points: progress.points + Math.max(0, xp),
  };
}
...
interface AppState extends PersistShape {
  ...
  /** Spend coins/gems to unlock a section/cosmetic. Returns false if blocked. */
  purchaseUnlock: (id: string, cost: UnlockCost) => boolean;
  /** Switch the active cosmetic accent theme (must be owned). */
...
export const useStore = create<AppState>((set, get) => ({
  ...
  claimReward: (id) => {
    const active = activeOf(get());
    const milestone = REWARD_MILESTONES.find((m) => m.id === id);
    if (!active || !milestone) return false;
    const p = active.progress;
    // Enforce the gate here too, not just in the UI: must have reached the
    // point threshold and must not have claimed this milestone before.
    if (p.points < milestone.points || p.claimedRewards.includes(id)) return false;

    set((s) =>
      mutateActive(s, (prog) => {
        const rewarded = applyXp(prog, milestone.reward.xp, milestone.reward.coins);
        return {
          ...rewarded,
          gems: rewarded.gems + milestone.reward.gems,
          claimedRewards: [...prog.claimedRewards, id],
        };
      }),
    );
    void syncProgress(get);
  ...
  purchaseUnlock: (id, cost) => {
    ...
    const p = active.progress;
    if (p.unlocks.includes(id)) return false; // already owned
    if (cost.minLevel && p.level < cost.minLevel) return false;
    if (p.coins < cost.coins || p.gems < (cost.gems ?? 0)) return false;

    set((s) =>
      mutateActive(s, (prog) => ({
        ...prog,
        coins: prog.coins - cost.coins,
        gems: prog.gems - (cost.gems ?? 0),
        unlocks: [...prog.unlocks, id],
      })),
    );
    void syncProgress(get);
    return true;
  ...
  setAccent: (id) => {
    set((s) => mutateActive(s, (p) => ({ ...p, accent: id })));
    void syncProgress(get);
  },
}));
...
export const useHasUnlock = (id: string) =>
  useStore((s) => activeOf(s)?.progress.unlocks.includes(id) ?? false);
